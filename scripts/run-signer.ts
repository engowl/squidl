...
async function main() {
  ...
  try {
    ...
    const [address1, ePub1, tag1] = await stealthSigner.connect(ethers.provider).generateStealthAddress.staticCall()
    console.log([address1, ePub1, tag1]);
    console.log(await stealthSigner.checkStealthAddress.staticCall(ePub1, tag1));
    const [key1, addr1] = await stealthSigner.computeStealthKey.staticCall(ePub1);
    ...
    const [address2, ePub2, tag2] = await stealthSigner.connect(ethers.provider).generateStealthAddress.staticCall()
    console.log([address2, ePub2, tag2]);
    console.log(await stealthSigner.checkStealthAddress.staticCall(ePub2, tag2));
    const [key2, addr2] = await stealthSigner.computeStealthKey.staticCall(ePub2);
  ...
  await (await stealthSigner.generateStealthAddress()).wait(); // Reveal the secret.
  const address = await stealthSigner.generateStealthAddress.staticCallResult(); // Get the value.
