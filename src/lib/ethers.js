...
async function main() {
  ...
  const [metaAddress, spendPub, viewingPub] =
    await stealthSigner.getMetaAddress.staticCall(auth, metaAddressIndex);
  const labelIndex = 0;
  ...
  try {
    ...
    const [address1, ePub1, tag1] =
      await stealthSigner.generateStealthAddress.staticCall(
        metaAddress,
        labelIndex
      );
    console.log([address1, ePub1, tag1]);
    console.log(
      await stealthSigner.checkStealthAddress.staticCall(
        metaAddress,
        labelIndex,
        ePub1,
        tag1
    ...
    const [key1, addr1] = await stealthSigner.computeStealthKey.staticCall(
      auth,
      metaAddress,
      labelIndex,
      ePub1
    ...
    if (wallet1.address !== address1) {
      console.log("Uh oh. The stealth key is incorrect!", [
        address1,
        wallet1.address,
      ]);
      process.exit(1);
    ...
    const [address2, ePub2, tag2] = await stealthSigner
      ...
      .generateStealthAddress.staticCall(metaAddress, labelIndex);
    ...
    const [key2, addr2] = await stealthSigner.computeStealthKey.staticCall(
      auth,
      metaAddress,
      labelIndex,
      ePub2
  ...
  await (
    await stealthSigner.generateStealthAddress(metaAddress, labelIndex)
  ).wait(); // Reveal the secret.
  const address = await stealthSigner.generateStealthAddress.staticCallResult(
    metaAddress,
    labelIndex
  ); // Get the value.
...
export async function generateTraffic(n) {
